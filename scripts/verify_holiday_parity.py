#!/usr/bin/env python3
"""
MetroPulse Phase 3B — 連假 range 正確性驗證
================================================================

與 verify_year_parity.py 同一套檢查邏輯，套用在 holiday range（range_id=holiday:{event_key}:{year}）：

  1. holiday OD aggregate correctness：range_od_flow 的加總與 daily_od_flow 對該連假
     [start_date, end_date] 的直接聚合完全一致——兩種獨立 SQL 聚合路徑互相驗證。
  2. holiday PageRank 全站 correctness：每個時段都有全部站點的列、pr_value 總和為 1.0、
     pr_rank 是 1..N 無重複的完整排列、normalized_score 端點正確。
  3. date_ranges 有正確持久化 is_complete/day_count/expected_day_count/start_date/end_date。

用法：
  python3 scripts/verify_holiday_parity.py --event-key lunar-new-year --year 2026 --local
  python3 scripts/verify_holiday_parity.py --event-key test-holiday --year 2030 --local --expect-incomplete
"""
...
def main():
    parser = argparse.ArgumentParser(description='驗證連假 range 聚合與 PageRank 的正確性')
    ...
    parser.add_argument('--expect-incomplete', action='store_true',
                         help='反例模式：只驗證這個連假被正確判定為不完整，不跑 OD/PageRank 檢查')
    ...
    remote = args.remote
    range_id = f"holiday:{args.event_key}:{args.year}"
    ...
    print(f"\n{'='*70}")
    print(f"  連假 range 正確性驗證：{args.event_key}:{args.year}（range_id={range_id}）")
    print(f"{'='*70}\n")

    print("[0/3] holiday_events + date_ranges 狀態記錄")
    ev_rows = d1_query(f"SELECT * FROM holiday_events WHERE event_key='{args.event_key}' AND year={args.year}", project_root, remote, args.db_name)
    if not ev_rows:
        check("holiday_events 存在", False, "找不到這個 event_key/year，後續檢查無法進行")
        sys.exit(1)
    ev = ev_rows[0]
    check("holiday_events 存在", True, f"{ev['name_zh']}：{ev['start_date']} ~ {ev['end_date']}")
    ...
    if not dr_rows:
        ...
        sys.exit(1)
    dr = dr_rows[0]
    check("date_ranges.range_type = 'holiday'", dr['range_type'] == 'holiday', dr['range_type'])
    check("date_ranges.holiday_event_id = event_key", dr['holiday_event_id'] == args.event_key, dr['holiday_event_id'])
    ...
    if args.expect_incomplete:
        ...
        check("不完整連假沒有寫入 range_od_flow", od_count == 0, f"實際筆數：{od_count}")
        check("不完整連假沒有寫入 range_pagerank", pr_count == 0, f"實際筆數：{pr_count}")
        print(f"\n{'='*70}")
        all_ok = len(report['failures']) == 0
        print(f"  結論：{'PASS — 反例確實被判定為不完整，且沒有污染 range_* 表' if all_ok else 'FAIL'}")
        print(f"{'='*70}\n")
        report_path = os.path.join(project_root, 'scripts', 'output', f"holiday_parity_report_{args.event_key}_{args.year}_incomplete_{'remote' if remote else 'local'}.json")
    ...
    check("date_ranges.start_date 對齊 holiday_events", dr['start_date'] == ev['start_date'], dr['start_date'])
    check("date_ranges.end_date 對齊 holiday_events", dr['end_date'] == ev['end_date'], dr['end_date'])
    check("date_ranges.computed_at 有值", bool(dr['computed_at']), dr['computed_at'])

    start_date, end_date = ev['start_date'], ev['end_date']

    print("\n[1/3] holiday OD aggregate correctness")
    direct_totals = {r['period']: r['total'] for r in d1_query(
        f"SELECT period, SUM(flow_count) as total FROM daily_od_flow "
        f"WHERE service_date BETWEEN '{start_date}' AND '{end_date}' GROUP BY period",
    ...
    print("\n[2/3] holiday PageRank 全站 correctness")
    ...
    print(f"\n{'='*70}")
    all_ok = len(report['failures']) == 0
    print(f"  結論：{'PASS — 連假 range 聚合與 PageRank 通過所有正確性檢查' if all_ok else 'FAIL — 見上方標記為 FAIL 的項目'}")
    print(f"{'='*70}\n")

    report_path = os.path.join(project_root, 'scripts', 'output', f"holiday_parity_report_{args.event_key}_{args.year}_{'remote' if remote else 'local'}.json")
    os.makedirs(os.path.dirname(report_path), exist_ok=True)
    with open(report_path, 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    print(f"報告已寫入：{report_path}")
